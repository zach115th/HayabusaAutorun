...
.DESCRIPTION
    ...
    4. Executes Hayabusa scan (CSV, Timesketch or JSONL) against local EVTX files.
    5. Optionally invokes Takajo to parse JSONL for deep artifact extraction.
    6. Informs user of completion and result location.
.NOTES
    Requires Hayabusa v4.0.0+:
      - csv-timeline / json-timeline are combined into 'dfir-timeline';
        output format is selected with -t/--output-type (csv|json|jsonl).
      - The old -L/--JSONL-output flag is removed (replaced by -t jsonl).
      - Long option names are now lowercase (--UTC is now --utc).
      - -t no longer means --threads; use the long form --threads if needed.
.PARAMETER None
.VERSION
    20260904r1
#>
    ...
    switch ($Profile.ToUpper()) {
        'C' {
            ...
            Start-Process -FilePath $Executable -ArgumentList `
                "dfir-timeline -t csv --sort --directory $quotedLogs --scan-all-evtx-files --no-wizard --enable-all-rules --multiline --output $csvOut --profile super-verbose --remove-duplicate-detections --utc -N -q" `
                -NoNewWindow `
                -PassThru `
                -Wait
        }
        'T' {
            ...
            Start-Process -FilePath $Executable -ArgumentList `
                "dfir-timeline -t csv --sort --directory $quotedLogs --scan-all-evtx-files --no-wizard --enable-all-rules --output $tsOut --profile timesketch-verbose --remove-duplicate-detections --utc -N -q" `
                -NoNewWindow `
                -PassThru `
                -Wait
        }
        'P' {
            ...
            Write-Host "`n`tStarting JSONL timeline scan...`n" -ForegroundColor Green
            Start-Process -FilePath $Executable -ArgumentList `
                "dfir-timeline -t jsonl --sort --directory $quotedLogs --scan-all-evtx-files --no-wizard --enable-all-rules -o $jsonlOut --profile super-verbose --remove-duplicate-detections --utc -N -q" `
                -NoNewWindow `
                -PassThru `
                -Wait
        }
